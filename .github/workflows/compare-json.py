...
projects = {
    "rodina": {
        ...
        "mod": "https://radarebot.hhos.net/rod_modclient",
        "package": "com.rodina.game"
    },
    "arizona": {
        "original": "https://mob.maz-ins.com/game/release",
        "mod": "https://radarebot.hhos.net/arz_modclient",
...
def merge_json_files(json_files):
    ...
    def process_item(item, current_dir=None):
        ...
        if item.get("type") == "file":
            ...
            if "data" in item:
                for subitem in item["data"]:
                    process_item(subitem, target_dir)
...
def process_project(project_name):
    if project_name not in projects:
        print(f"Проект {project_name} не найден в списке проектов")
        return [], []
    ...
    for file_type in file_types:
        ...
        try:
            print(f"Загрузка {file_type} JSON с {original_url}")
            original_data = download_json(original_url)
            original_jsons.append(original_data)
            
            print(f"Загрузка {file_type} JSON с {arzmod_url}")
    ...
    for file_info in all_files:
        ...
        print(f"DOWNLOAD: {download_url} {file_path}")
