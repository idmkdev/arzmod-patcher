...
public class UpdateServicePatch {
        ...
        final Runnable retryRunnable = new Runnable() {
            ...
            public void run() {
                if (attempts[0] < maxAttempts) {
                    ...
                    HomeExternalUiState currentValue;
                    do {
    ...
    public static void setHomeUi(boolean check) {
        retrySetHomeUi(check, 3);
    }

    public static boolean isModeMods() {
        if(!SettingsPatch.getSettingsKeyValue(SettingsPatch.IS_MODE_MODS))
        {
            setHomeUi(false);
            return true;
        } 
